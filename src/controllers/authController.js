...
exports.register = async (req, res) => {
    try {
        // Accept optional schoolId from client for parent/teacher/student registration flows
        const {
            email,
            password,
            firstName,
        ...
        if (role === 'director') {
            ...
            // Validate provided schoolId for non-director roles
        ...
        // Create user, attach school if available
        ...
        // Director post-creation actions
        ...
        // Parent: create Parent doc, attach school if present (school optional)
        if (role === 'parent') {
            const parentPayload = {
                user: newUser._id,
                phone: phone || '',
        ...
        // Teacher: create Teacher doc, school optional; auto-approve if school present
        if (role === 'teacher') {
        ...
        // Student: create temporary registration; school optional; auto-approve if school present
        if (role === 'student') {
        ...
        // Respond according to approval state
        ...
        if (error.code === 11000) {
            ...
            const message = field === 'email' ? 'Cette adresse email est déjà utilisée' :
                field === 'teacherId' ? 'Cet ID enseignant existe déjà' :
                    field === 'studentId' ? 'Cet ID étudiant existe déjà' :
                        'Cette valeur existe déjà';
            return res.status(400).json({ status: 'error', message });
        }

        // Validation error handling (Mongoose)
...
exports.protect = async (req, res, next) => {
    try {
        ...
        if (!currentUser) {
            return res.status(401).json({ status: 'error', message: 'L\'utilisateur associé à ce token n\'existe plus.' });
        }

        if (currentUser.registrationStatus !== 'approved') {
            return res.status(401).json({ status: 'error', message: 'Votre compte est en attente d\'approbation' });
        }

        if (!currentUser.isActive) {
            return res.status(401).json({ status: 'error', message: 'Votre compte est désactivé' });
        }
        ...
        res.status(401).json({ status: 'error', message: 'Erreur d\'authentification: ' + error.message });
    }
};
...
exports.approveRegistration = async (req, res) => {
    try {
        ...
        if (!school) {
            return res.status(404).json({ status: 'error', message: 'École non trouvée' });
        }
        ...
        if (user.role === 'teacher') {
            ...
            if (teacher) {
                ...
                return res.status(404).json({ status: 'error', message: 'Enregistrement enseignant non trouvé' });
            }
        } else if (user.role === 'student') {
            ...
            if (studentReg) {
                ...
                return res.status(404).json({ status: 'error', message: 'Enregistrement d\'inscription étudiant non trouvé' });
            }
        }
        ...
        if (error.code === 11000) {
            return res.status(400).json({ status: 'error', message: 'Erreur de duplication: Cet ID existe déjà' });
        }

        res.status(400).json({ status: 'error', message: 'Erreur lors de l\'approbation: ' + error.message });
    }
};
...
exports.rejectRegistration = async (req, res) => {
    try {
        ...
        if (user.role === 'teacher') {
            await Teacher.findOneAndUpdate({ user: userId }, { registrationStatus: 'rejected' });
        } else if (user.role === 'student') {
            await StudentRegistration.findOneAndUpdate({ user: userId }, { registrationStatus: 'rejected' });
        }
        ...
        res.status(400).json({ status: 'error', message: 'Erreur lors du rejet: ' + error.message });
    }
};
